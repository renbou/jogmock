...
func TestEncoding(t *testing.T) {
	...
	// uint8
	currentEncoder = "EncodeFitUint8"
	currentEndianness = LittleEndian
	b = EncodeFitUint8(nil, LittleEndian, 0x69)
	assert(uint8(0x69), []byte{0x69})

	currentEndianness = BigEndian
	b = EncodeFitUint8(nil, BigEndian, 0x42)
	assert(uint8(0x42), []byte{0x42})

	// sint8
	currentEncoder = "EncodeFitSint8"
	currentEndianness = LittleEndian
	b = EncodeFitSint8(nil, LittleEndian, 100)
	...
	currentEndianness = BigEndian
	b = EncodeFitSint8(nil, BigEndian, 34)
	...
	// enum
	currentEncoder = "EncodeFitEnum"
	currentEndianness = LittleEndian
	b = EncodeFitEnum(nil, LittleEndian, 0)
	assert(FitEnum(0), []byte{0})

	currentEndianness = BigEndian
	b = EncodeFitEnum(nil, BigEndian, 19)
	assert(FitEnum(19), []byte{0x13})

	// uint16
	currentEncoder = "EncodeFitUint16"
	currentEndianness = LittleEndian
	b = EncodeFitUint16(nil, LittleEndian, 0xA932)
	assert(uint16(0xA932), []byte{0x32, 0xA9})

	currentEndianness = BigEndian
	b = EncodeFitUint16(nil, BigEndian, 0xF243)
	assert(uint16(0xF243), []byte{0xF2, 0x43})

	// sint16
	currentEncoder = "EncodeFitSint16"
	currentEndianness = LittleEndian
	b = EncodeFitSint16(nil, LittleEndian, 31234)
	assert(int16(31234), []byte{0x02, 0x7a})

	b = EncodeFitSint16(nil, LittleEndian, -25043)
	assert(int16(-25043), []byte{0x2d, 0x9e})

	currentEndianness = BigEndian
	b = EncodeFitSint16(nil, BigEndian, 23984)
	assert(int16(23984), []byte{0x5d, 0xb0})

	b = EncodeFitSint16(nil, BigEndian, -12398)
	assert(int16(-12398), []byte{0xcf, 0x92})

	// uint32
	currentEncoder = "EncodeFitUint32"
	currentEndianness = LittleEndian
	b = EncodeFitUint32(nil, LittleEndian, 0xEF73DAB8)
	assert(uint32(0xEF73DAB8), []byte{0xB8, 0xDA, 0x73, 0xEF})

	currentEndianness = BigEndian
	b = EncodeFitUint32(nil, BigEndian, 0x18B3EF73)
	assert(uint32(0x18B3EF73), []byte{0x18, 0xB3, 0xEF, 0x73})

	// sint32
	currentEncoder = "EncodeFitSint32"
	currentEndianness = LittleEndian
	b = EncodeFitSint32(nil, LittleEndian, 234763279)
	assert(int32(234763279), []byte{0x0f, 0x34, 0xfe, 0x0d})

	b = EncodeFitSint32(nil, LittleEndian, -32776438)
	assert(int32(-32776438), []byte{0x0a, 0xdf, 0x0b, 0xfe})

	currentEndianness = BigEndian
	b = EncodeFitSint32(nil, BigEndian, 974326637)
	assert(int32(974326637), []byte{0x3a, 0x13, 0x0b, 0x6d})

	b = EncodeFitSint32(nil, BigEndian, -1283723832)
	assert(int32(-1283723832), []byte{0xb3, 0x7b, 0xed, 0xc8})

	// uint64
	currentEncoder = "EncodeFitUint64"
	currentEndianness = LittleEndian
	b = EncodeFitUint64(nil, LittleEndian, 0xEF73DAB818B3EF73)
	assert(uint64(0xEF73DAB818B3EF73), []byte{0x73, 0xEF, 0xB3, 0x18, 0xB8, 0xDA, 0x73, 0xEF})

	currentEndianness = BigEndian
	b = EncodeFitUint64(nil, BigEndian, 0x18B3EF73EF73DAB8)
	assert(uint64(0x18B3EF73EF73DAB8), []byte{0x18, 0xB3, 0xEF, 0x73, 0xEF, 0x73, 0xDA, 0xB8})

	// sint64
	currentEncoder = "EncodeFitSint64"
	currentEndianness = LittleEndian
	b = EncodeFitSint64(nil, LittleEndian, 2389749234789324833)
	assert(int64(2389749234789324833), []byte{0x21, 0x18, 0xe1, 0x81, 0x44, 0x18, 0x2a, 0x21})

	b = EncodeFitSint64(nil, LittleEndian, -984593745873495847)
	assert(int64(-984593745873495847), []byte{0xd9, 0x98, 0x23, 0x69, 0x34, 0x05, 0x56, 0xf2})

	currentEndianness = BigEndian
	b = EncodeFitSint64(nil, BigEndian, 1827367672434837284)
	assert(int64(1827367672434837284), []byte{0x19, 0x5c, 0x1d, 0x37, 0x5d, 0x82, 0xc7, 0x24})

	b = EncodeFitSint64(nil, BigEndian, -3746765475475764754)
	...
	currentEndianness = LittleEndian
	b = EncodeFitBaseType(nil, LittleEndian, FIT_TYPE_FLOAT64)
	assert(FIT_TYPE_FLOAT64, []byte{uint8(FIT_TYPE_FLOAT64)})

	currentEndianness = BigEndian
	b = EncodeFitBaseType(nil, BigEndian, FIT_TYPE_UINT64)
