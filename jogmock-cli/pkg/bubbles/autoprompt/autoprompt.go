// creds to github.com/mritd/bubbles, this just adds filepath autocompletion
...
func VFPathExists(path string) error {
...
func (m *Model) Update(msg tea.Msg) (*Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case tea.KeyTab:
			...
			if suggest := autoSuggestFile(m.input.Value()); suggest != "" {
				m.err = m.ValidateFunc(suggest)
				m.input.SetValue(suggest)
				m.input.SetCursor(len(suggest))
				return m, nil
			}
		case tea.KeyRunes:
