...
func NewActivity(options *ActivityOptions) (*StravaActivity, error) {
	...
	if options.StartTime.Year() < time.Now().Year()-10 ||
		options.StartTime.Year() > time.Now().Year() {
		return nil, errors.New("strange start time year")
	}
...
func (act *StravaActivity) generateNewWave() {
	...
	if rand.Intn(10) == 0 {
		...
		rnd := randFloat64InRange(0, 2)
		if rnd <= 1 {
			act.waveCurSlope = -0.7 - rnd
		} else {
			act.waveCurSlope = 0.7 + (rnd - 1)
		}

		// from 15 to 35 seconds, faster than usual
		// act.waveNumSegments = randIntInRange(15, 35)
		act.waveNumSegments = randIntInRange(25, 50)
	} else {
		// slope in [-0.5, 0.5]
		act.waveCurSlope = randFloat64InRange(-0.7, 0.7)
		// act.waveCurSlope = randFloat64InRange(-0.3, 0.3)

		// from 50 to 90 seconds
		// act.waveNumSegments = randIntInRange(50, 90)
		act.waveNumSegments = randIntInRange(15, 40)
	}
